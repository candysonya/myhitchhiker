...
export const AddApiTabType = 'add API tab';

export const TriggerTabType = 'trigger tab from API';
...
export const AlreadySaveRecordType = 'mark record as already saved';
...
function* pushSaveRecordToChannel(action: any) {
	try {
		const res = yield call(action.value.isNew ? RequestManager.post : RequestManager.put, Urls.getUrl(`record`), { ...action.value.record, history: [] });
		yield res.json();
		return;
	} catch (err) {
		return;
