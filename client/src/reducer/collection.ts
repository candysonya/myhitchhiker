...
import { ActiveTabType, ActiveRecordType, DeleteRecordType, MoveRecordType, SendRequestFulfilledType, AddTabType, AddApiTabType, TriggerTabType, RemoveTabType, SendRequestType, CancelRequestType, SaveRecordType, SaveAsRecordType, AlreadySaveRecordType, ChangeCurrentParamType, SendRequestForParamType, SendRequestForParamFulfilledType, UpdateDisplayRecordPropertyType } from '../action/record';
...
import {
	...
	RecordState,
	DisplayRecordsState,
	displayRecordsDefaultValue,
	getNewRecordState,
	getNewApiRecordState,
	getExistingRecordState
...
import { SyncCollectionType } from "../action/api";
...
export function collectionState(state: CollectionState = collectionDefaultValue, action: any): CollectionState {
    switch (action.type) {
	    ...
	    case SyncCollectionType: {
		    const collectionsInfo = action.value.result as DtoCollectionWithRecord;
		    return { ...state, collectionsInfo, fetchCollectionState: { status: RequestStatus.success } };
	    }
...
export function recordStates(states: _.Dictionary<RecordState> = displayRecordsDefaultValue.recordStates, action: any): _.Dictionary<RecordState> {
    switch (action.type) {
	    ...
	    case AlreadySaveRecordType: {
		    const { record } = action.value;
		    const { id, name } = record;
		    return { ...states, [id]: { ...states[id], name, isChanged: false, conflictType: ConflictType.none } };
	    }
        case MoveRecordType: {
...
export function recordWithResState(state: DisplayRecordsState = displayRecordsDefaultValue, action: any): DisplayRecordsState {
    ...
    switch (action.type) {
	    ...
	    case AddApiTabType: {
		    let newRecordState = getNewApiRecordState();
		    if (!!action.value) {
		    	newRecordState.record.apiId = action.value;
		    }
		    return {
			    ...state,
			    recordStates: {
	    ...
	    case TriggerTabType: {
		    let newRecordState = getNewRecordState();
		    newRecordState.record.apiId = action.value;
		    return {
			    ...state,
			    recordStates: {
